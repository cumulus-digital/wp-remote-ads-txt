...
if (!\class_exists('CUMULUS\Wordpress\RemoteAdsTxt\Vendors\WordPressSettingsFramework')) {
    ...
    class WordPressSettingsFramework
    {
         ...
         * @var array
         */
        protected $setting_defaults = array('id' => 'default_field', 'title' => 'Default Field', 'desc' => '', 'std' => '', 'type' => 'text', 'placeholder' => '', 'choices' => array(), 'class' => '', 'subfields' => array());
        /**
        ...
        {
            $defaults = array('parent_slug' => \false, 'page_slug' => '', 'page_title' => '', 'menu_title' => '', 'capability' => 'manage_options');
            $args = \wp_parse_args($args, $defaults);
            $this->settings_page['title'] = $args['page_title'];
            $this->settings_page['capability'] = $args['capability'];
            if ($args['parent_slug']) {
                \add_submenu_page($args['parent_slug'], $this->settings_page['title'], $args['menu_title'], $args['capability'], $this->settings_page['slug'], array($this, 'settings_page_content'));
            } else {
                \add_menu_page(
        ...
        public function admin_enqueue_scripts()
        {
            ...
            \wp_register_script('jquery-ui-timepicker', $this->options_url . $jqtimepicker_js_path, array('jquery', 'jquery-ui-core'), \filemtime($this->options_path . $jqtimepicker_js_path), \true);
            $wpsf_js_path = 'assets/js/main.js';
            \wp_register_script('wpsf', $this->options_url . $wpsf_js_path, array('jquery'), \filemtime($this->options_path . $wpsf_js_path), \true);
            ...
            $data = array('select_file' => \esc_html__('Please select a file to import', 'wpsf'), 'invalid_file' => \esc_html__('Invalid file', 'wpsf'), 'something_went_wrong' => \esc_html__('Something went wrong', 'wpsf'));
            \wp_localize_script('wpsf', 'wpsf_vars', $data);
            // Styles.
            $jqtimepicker_css_path = 'assets/vendor/jquery-timepicker/jquery.ui.timepicker.css';
            \wp_register_style('jquery-ui-timepicker', $this->options_url . $jqtimepicker_css_path, array(), \filemtime($this->options_path . $jqtimepicker_css_path));
            $wpsf_css_path = 'assets/css/main.css';
            \wp_register_style('wpsf', $this->options_url . $wpsf_css_path, array(), \filemtime($this->options_path . $wpsf_css_path));
            $jqui_css_path = 'assets/vendor/jquery-ui/jquery-ui.css';
            \wp_register_style('jquery-ui-css', $this->options_url . $jqui_css_path, array(), \filemtime($this->options_path . $jqui_css_path));
                            ...
                            foreach ($section['fields'] as $field) {
                                if (isset($field['id']) && $field['id'] && isset($field['title'])) {
                                    ...
                                    if (isset($field['link']) && \is_array($field['link'])) {
                                        ...
                                        $link_external = isset($field['link']['external']) ? (bool) $field['link']['external'] : \true;
        ...
        public function generate_import_field($args)
        {
            ...
            echo \sprintf('
				...
				</div>', \esc_attr($args['id']), \esc_attr($args['id']), \esc_attr($args['value']), \esc_attr(\wp_create_nonce('wpsf_import_settings')), \esc_attr($this->option_group));
            $this->generate_description($args['desc']);
        }
        /**
        ...
        public function generate_group_field($args)
        {
            ...
            \printf(
                ...
                \esc_attr($args['id']),
                // @codingStandardsIgnoreStart
                $this->generate_group_row_template($args, \true)
            );
            $this->generate_description($args['desc']);
        }
        /**
        ...
        public function generate_image_checkboxes_field($args)
        {
            ...
            foreach ($args['choices'] as $value => $choice) {
                ...
                $is_checked = \is_array($args['value']) && \in_array($value, $args['value'], \true);
                $checked_class = $is_checked ? 'wpsf-visual-field__item--checked' : '';
                echo \sprintf('<li class="wpsf-visual-field__item %s">
						<label>
					...
					</li>', \esc_attr($checked_class), \esc_url($choice['image']), \esc_attr($args['name']), \esc_attr($field_id), \esc_attr($value), \esc_attr($args['class']), \checked(\true, $is_checked, \false), \esc_attr($choice['text']));
            }
            echo '</ul>';
            $this->generate_description($args['desc']);
        }
        ...
        public function generate_image_radio_field($args)
        {
            ...
            foreach ($args['choices'] as $value => $choice) {
                ...
                echo \sprintf('<li class="wpsf-visual-field__item %s">				
					...
					</li>', $checked ? 'wpsf-visual-field__item--checked' : '', \esc_attr($choice['image']), \esc_attr($args['name']), \esc_attr($field_id), \esc_attr($value), \esc_attr($args['class']), \esc_html($checked), \esc_attr($choice['text']));
            }
            echo '</ul>';
            $this->generate_description($args['desc']);
        }
            ...
            foreach ($args['choices'] as $value => $text) {
                if (\is_array($text)) {
                    ...
                    foreach ($text as $group_value => $group_text) {
                        $selected = \in_array((string) $group_value, $values, \true) ? ' selected="selected" ' : '';
                        echo \sprintf('<option value="%s" %s>%s</option>', \esc_attr($group_value), \esc_html($selected), \esc_html($group_text));
                    }
                    echo '</optgroup>';
                    continue;
                }
                $selected = \in_array((string) $value, $values, \true) ? ' selected="selected" ' : '';
        ...
        public function generate_checkboxes_field($args)
        {
            ...
            foreach ($args['choices'] as $value => $text) {
                $checked = \is_array($args['value']) && \in_array(\strval($value), \array_map('strval', $args['value']), \true) ? 'checked="checked"' : '';
                $field_id = \sprintf('%s_%s', $args['id'], $value);
        ...
        public function generate_code_editor_field($args)
        {
            \printf('<textarea
					...
					cols="60"
					class="%s"
				>%s</textarea>', \esc_attr($args['name']), \esc_attr($args['id']), \esc_attr($args['placeholder']), \esc_attr($args['class']), \esc_html($args['value']));
        ...
        public function generate_custom_field($args)
        {
            ...
            // This output isn't easily escaped.
            // @codingStandardsIgnoreEnd
        }
        /**
        ...
        public function settings()
        {
             ...
             * @param boolean
             */
            if (\apply_filters('wpsf_show_save_changes_button_' . $this->option_group, \true)) {
                ?>
					<p class="submit">
        ...
        public function tab_links()
        {
             ...
             * @param boolean
             */
            if (!\apply_filters('wpsf_show_tab_links_' . $this->option_group, \true)) {
                return;
            }
            /**
            ...
            foreach ($this->tabs as $tab_data) {
                ...
                echo \esc_attr($tab_data['id']);
                ?>"><?php 
                echo \wp_kses_post($tab_data['title']);
                ?></a>
					</li>
					<?php 
                $i++;
        ...
        public static function export_settings()
        {
            ...
            // @codingStandardsIgnoreStart
            echo $options;
            // The string is already encoded, and option values will have already been escaped.
            // @codingStandardsIgnoreEnd
            exit;
        }
        /**
        ...
        public function import_settings()
        {
            ...
            if (!\is_string($settings) || !\is_array(\json_decode($settings, \true))) {
                \wp_send_json_error();
            }
            $settings_data = \json_decode($settings, \true);
